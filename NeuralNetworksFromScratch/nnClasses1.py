...
class Optimizer_SGD:
    ...
    def pre_update_params(self):
        # Call once before any parametr updates
        if self.decay:
    ...
    def update_params(self, layer):
        ...
        if self.momentum:
            ...
            if not hasattr(layer, 'weight_momentums'):
                ...
                # If there is no momentum array for weights, the array for biases doesn't yet exist too.
                layer.bias_momentums = np.zeros_like(layer.biases)

            # Build weight updates with momentum - take perious updates mu;tipled by retain factor and
...
class Optimizer_Adagrad:
    ...
    def pre_update_params(self):
        ...
        if self.decay:
            self.current_learning_rate = self.learning_rate * (1. / (1. + self.decy * self.iterations))

    def update_params(self, layer):

        # if layer doesn't contain cache arrays, create them filled w/zeros
        ...
        # Vanilla SGD parameter update + normization w/square rooted cache
        layer.weights += -self.current_learning_rate * layer.dweights / (np.sqrt(layer.weight_cache) + self.epsilon)
        layer.biases += -self.current_learning_rate * layer.dbiases / (np.sqrt(layer.bias_cache) + self.epsilon)

    def post_update_params(self):
        # Call oncew after any parameter updates
        self.iterations += 1
