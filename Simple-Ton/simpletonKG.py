...
input_aliveKG = "processedData/aliveKG.gml"
input_deviceKG = "processedData/deviceKG.gml"
input_spaceKG = "processedData/spaceKG.gml"
"""
# Define methods for the dynamic class
def dynamic_init(self, name, status):
    self.name = name
    self.status = status

def dynamic_greet(self):
    return f"Hello from DynamicClass with value: {self.name}"

# Create the class dynamically
DynamicClass = type(
    "DynamicClass",  # Name of the class
    (object,),       # Base classes (inherits from object)
    {
        "__init__": dynamic_init,  # Constructor method
        "greet": dynamic_greet,    # Regular method
...
def loadKG():
    ...
    for kg in simpKG:
        ...
        if kg.name == 'Alive' and kg.number_of_nodes() == 0:
            ...
            print('Graph object name: ', kg.name)
            print(f'{kg.name} has {kg.number_of_nodes()}')
