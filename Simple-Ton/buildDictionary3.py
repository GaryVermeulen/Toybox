        ...
        with open(dataPath + '/' + inFile, 'r', encoding="latin-1") as f:
            while (line := f.readline()):
                if len(line) > 1:
                    ...
                    # Now seperate pos from definition
                    index = definition.find(')')
                    docPOS = definition[0:index + 1]
                    
                    # Build new row
                    rows.append((word, docPOS, definition[index + 1:].lstrip()))
                    """
                    dStop += 1
                    if dStop > 5:
...
def buildMongoCollection(dictionary):
    ...
    for line in dictionary:
        ...
        wordDef = {'word': line[0], 'pos': line[1], 'definition': line[2]}
